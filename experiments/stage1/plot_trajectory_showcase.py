...
def bin_mean(c,p):
    ...
    return out
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--trajs",nargs="+",type=int,default=[4,13]); ap.add_argument("--seed",type=int,default=0); ap.add_argument("--split",choices=["val","train"],default="val"); a=ap.parse_args()
    ...
    jdir = OUT if a.split=="val" else OUT/"kin_train"; ndir = OUT if a.split=="val" else OUT/"kin_train"; kdir = OUT/"kin" if a.split=="val" else OUT/"kin_train"
    summ={arm: {r["traj"]:r for r in json.load(open(jdir/f"{arm}_s{a.seed}.json"))["per_traj"]} for arm in ARMS}
    for ti in a.trajs:
        ...
        for j in range(3):
            ...
            for arm in ARMS:
                z=np.load(ndir/f"{arm}_s{a.seed}.npz"); c=z[f"synergy_coeff_traj{ti}"]; p=z[f"phase_traj{ti}"]; M=bin_mean(c,p)
        ...
        # hand speed from kin
        ax=axes[3]; kin0=np.load(kdir/f"T0_s{a.seed}.npz"); site=[str(s) for s in kin0["site_names"]]; hand=site.index("right_hand_mimic"); dt=float(kin0["dt"])
        ...
        for arm in ARMS:
            S=np.load(kdir/f"{arm}_s{a.seed}.npz")[f"site_traj{ti}"]; hs=np.linalg.norm(np.diff(S[:,hand],axis=0),axis=1)/dt
        ...
        fig.suptitle(f"{a.split} trajectory {ti} ({RS.shape[0]} frames), seed {a.seed}: human reference vs five arms"); fig.tight_layout()
        out=FIG/f"showcase_{a.split}{ti}_s{a.seed}.png"; fig.savefig(out,dpi=140); plt.close(fig); print("wrote",out)
