...
Usage:  source configs/env.sh; CUDA_VISIBLE_DEVICES=0 .venv/bin/python experiments/stage1/capture_endpoint_kinematics.py [--only T0:0 T3:0]
"""
...
OUT = REPO / "outputs/stage1_endpoint_compare/kin"
ARMS = ("T0", "T1", "T2", "T3", "T4")
...
def capture(arm, seed, leaf, save_model: bool):
    import jax
    ...
    td = th.traj.data
    split = np.asarray(td.split_points)
    ...
    np.savez_compressed(OUT / f"{arm}_s{seed}.npz", **save)
    if save_model:
...
def main():
    ...
    for k, (arm, seed, leaf) in enumerate(leaves):
        if (OUT / f"{arm}_s{seed}.npz").exists():
            print(f"[skip] {arm} s{seed}", flush=True); continue
        capture(arm, seed, leaf, save_model=(k == 0 or not (OUT / "model.mjb").exists()))
    return 0
