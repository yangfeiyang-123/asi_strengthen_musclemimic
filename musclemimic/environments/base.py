...
from musclemimic.core.mujoco_mjx import Mjx, MjxAdditionalCarry, MjxState
...
from loco_mujoco.trajectory import Trajectory, TrajectoryHandler, TrajectoryTransitions, TrajState
...
class LocoCarry(MjxAdditionalCarry):
    ...
    Args:
        ...
        sampling_weights (jax.Array): Adaptive sampling weights for trajectories.
    ...
    selected_traj_idx: jax.Array = struct.field(
    ...
    ema_done_counts: jax.Array | None = None  # Shape (num_envs, n_traj) for count-EMA
    ema_early_counts: jax.Array | None = None  # Shape (num_envs, n_traj) for count-EMA
    # Reward curriculum: dynamic qvel weight (always scalar, never None)
    qvel_w_sum: jax.Array = struct.field(
...
class LocoEnv(Mjx):
    ...
    def _is_done(self, obs: np.ndarray, absorbing: bool, info: dict, data: MjData, carry: LocoCarry) -> bool:
        """
        ...
        if self._goal.requires_trajectory or self._reward_function.requires_trajectory:
            ...
            # Check if we have a valid trajectory state (not EmptyState)
            if hasattr(traj_state, "subtraj_step_no") and hasattr(traj_state, "traj_no"):
                done |= self.th.reached_trajectory_end(traj_state, np)
            else:
                # EmptyState means no trajectory loaded, so don't terminate based on trajectory
                done |= False
    ...
    def _update_info_dictionary(self, info: dict, obs: np.ndarray, data: MjData, carry: LocoCarry) -> dict:
        """Add trajectory state info for debugging."""
        ...
        if hasattr(traj_state, "traj_no") and hasattr(traj_state, "subtraj_step_no"):
            info["traj_no"] = int(traj_state.traj_no)
            info["subtraj_step_no"] = int(traj_state.subtraj_step_no)
            info["traj_len"] = int(self.th.len_trajectory(traj_state.traj_no))
        return info

    def _mjx_is_done(self, obs: jnp.ndarray, absorbing: bool, info: dict, data: Data, carry: LocoCarry) -> bool:
        """
        ...
        if self._goal.requires_trajectory or self._reward_function.requires_trajectory:
            ...
            # if the end of the trajectory is reached, if so, we set done to True
            traj_state = carry.traj_state
            reached_end_of_traj = self.th.reached_trajectory_end(traj_state, jnp)
    ...
    def _simulation_post_step(self, model: MjModel, data: MjData, carry: LocoCarry) -> tuple[MjData, LocoCarry]:
        """
    ...
    def _mjx_simulation_post_step(self, model: Model, data: Data, carry: LocoCarry) -> tuple[Data, LocoCarry]:
        """
        ...
        if self.th is not None:
            if self.th.traj.transitions is None:
                ...
                th = TrajectoryHandler(
                    model=self._model, traj=traj, control_dt=self.dt, random_start=False, fixed_start_conf=(0, 0)
                )
                ...
                for i in tqdm(range(self.th.n_trajectories), desc="Creating Transition Dataset"):
                    ...
                    data, carry = self.obs_container.reset_state(self, model, data, carry, np)
                    ...
                    for j in range(1, self.th.len_trajectory(i)):
                        ...
                        obs, carry = self._create_observation(model, data, carry)
                        obs, data, info, carry = self._step_finalize(obs, model, data, info, carry)
            ...
            raise ValueError(
                "No trajectory was passed to the environment. " "To create a dataset pass a trajectory first."
            )
    ...
    def play_trajectory(
        ...
        if record:
            ...
            fps = 1 / self.dt
            recorder = (
                VideoRecorder(fps=fps, **recorder_params) if recorder_params is not None else VideoRecorder(fps=fps)
            )
        else:
            recorder = None
            ...
            for j in tqdm(range(nspe), disable=quiet):
                if callback_class is None:
                    ...
                    self._model, self._data, self._additional_carry = callback_class(
                        self, self._model, self._data, traj_data_sample, self._additional_carry
                    )
                ...
                if from_velocity and subtraj_step_no != 0:
                    ...
                    qpos[~is_free_joint_qpos_quat] = [
                        qp + self.dt * qv
                        for qp, qv in zip(
                            self._data.qpos[~is_free_joint_qpos_quat], qvel[~is_free_joint_qvel_rotvec], strict=False
                        )
                    ]
    ...
    def play_trajectory_from_velocity(
        ...
        self.play_trajectory(
            n_episodes, n_steps_per_episode, True, render, record, recorder_params, callback_class, quiet, key
        )
    ...
    def _init_additional_carry(self, key: jax.Array, model: MjModel, data: MjData, backend: ModuleType) -> LocoCarry:
        """
        ...
        if hasattr(carry, "termination_threshold"):
            threshold = carry.termination_threshold
            if hasattr(self, "_terminal_state_handler") and hasattr(
                self._terminal_state_handler, "mean_site_deviation_threshold"
            ):
    ...
    def mjx_reset(
        ...
        Note: Trajectory must be converted to JAX format before JIT compilation
        (done in engine.instantiate_env). Converting inside this method would
        trigger TracerBoolConversionError since mjx_reset runs within JIT.
        ...
        """
        return super().mjx_reset(
            key,
            selected_traj_idx=selected_traj_idx,
        )

    def _reset_carry(self, model: MjModel, data: MjData, carry: LocoCarry) -> tuple[MjData, LocoCarry]:
        """
        ...
        if self.th is not None:
            data, carry = (
                self.th.reset_state(self, self._model, data, carry, np) if self.th is not None else (data, carry)
            )
    ...
    def _mjx_reset_carry(self, model: Model, data: Data, carry: MjxAdditionalCarry) -> tuple[Data, MjxAdditionalCarry]:
        ...
        if self.th is not None:
            data, carry = (
                self.th.reset_state(self, self._model, data, carry, jnp) if self.th is not None else (data, carry)
            )
    ...
    def _get_from_obs(self, obs: np.ndarray | jnp.ndarray, obs_name: str) -> np.ndarray | jnp.ndarray:
        """
    ...
    def generate(
        ...
        **kwargs,
        ...
        from loco_mujoco.task_factories import DefaultDatasetConf, ImitationFactory, RLFactory
    ...
    def get_default_xml_file_path(cls) -> str:
        ...
        """
        raise NotImplementedError(
            f"Please implement the default_xml_file_path property " f"in the {type(cls).__name__} environment."
        )
    ...
    def root_body_name(self) -> str:
        ...
        """
        raise NotImplementedError(
            f"Please implement the root_body_name " f"info property in the {type(self).__name__} environment."
        )
    ...
    def upper_body_xml_name(self) -> str:
        ...
        """
        raise NotImplementedError(
            f"Please implement the upper_body_xml_name property " f"in the {type(self).__name__} environment."
        )
    ...
    def root_height_healthy_range(self) -> tuple[float, float]:
        ...
        """
        raise NotImplementedError(
            f"Please implement the root_height_healthy_range property " f"in the {type(self).__name__} environment."
        )
    ...
    def _get_observation_specification(spec: MjSpec) -> list[Observation]:
        ...
        """
        raise NotImplementedError(
            f"Please implement the _get_observation_specification method " f"in the {type(spec).__name__} environment."
        )
    ...
    def _get_action_specification(spec: MjSpec) -> list[str]:
        ...
        """
        raise NotImplementedError(
            f"Please implement the _get_action_specification method " f"in the {type(spec).__name__} environment."
        )
    ...
    def _delete_from_spec(
        spec: MjSpec, joints_to_remove: list[str], actuators_to_remove: list[str], equ_constraints_to_remove: list[str]
    ):
        """
