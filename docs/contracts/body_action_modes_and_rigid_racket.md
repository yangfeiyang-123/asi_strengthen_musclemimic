...
> [!NOTE]
> 主线只使用 `full_354`。`fixed_synergy` / `fixed_synergy_residual` 属于已归档的动作空间消融线
> （相关 runbook 见 `docs/archive/`），本文保留其合同仅为复现。刚性球拍 attachment 合同仍是 Stage 2/3 的生产依赖。
...
- `experiments/stage3/direct_residual/incoming_shuttle_hit_full354_v1.yaml`：策略直接输出 354-D，
  `stage3_lab.enabled=false`，不需要 latent checkpoint；
- `experiments/stage3/lab/incoming_shuttle_hit_impact_recovery_v2.yaml`：固定协同/latent
