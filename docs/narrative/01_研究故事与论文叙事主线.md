...
| （扩展）双人对打 | `environment/double_play/` DoublePlayRallyEnv，镜像自博弈 | 阶段三之后的评估场与自博弈扩展，不是主结果 |
...
| Stage 3 v3–v45 direct-residual 配置 | 工程探索记录，不进入主方法图 |

上述模块的文档已从 `docs/` 主目录移入 `docs/archive/`，代码保留但默认不排期。
...
### 已有能力（代码与合同，截至 2026-09-18）

- body/racket 分阶段 tracking，Stage 1 T0–T4 matched family 配置与 gate；
- 在线 15 通道 EMG anchor/synergy 奖励（fail-closed，需 verified tube）；
- teacher–student BC、DAgger、PPO 的 direct lifecycle；
- posterior(state, reference, EMG context)/prior/decoder latent、SynergyHead、context dropout、shuffle 负对照；
- LAB、Stage 3 action router、reachability gate 与 feed bank；
- 双人对打环境（CPU MuJoCo）。

### 尚不能声称完成

- Stage 1 正式 80/20 family：15 个 run 中 0 个到达 800M endpoint（进度见 `experiments/EXPERIMENT_LOG.md`）；
  已有的 T2/T3/T4 320M/640M 结果来自 40/10 子集、单 seed、无 T0，只能标为探索性；
- Stage 2：direct 与 latent family 一个 run 都没有；
- Stage 3：LAB 路径没有训练 run；direct-residual 线未产生稳定过网；
- impact 事件独立证据为 0，肌电只有单 subject 单 session，与视频受试者非配对；
- 完整的主方法消融和统计结论。
...
- 汇报 PPT：`进展/工作汇报0816.pdf`（主线定义）；
- 方法细节与消融命名：`02_三阶段方法与肌电参与机制.md`；
- 当前状态与待办：`03_当前状态与待办.md`；
- 正式实验合同：`docs/plans/PEASD正式实验计划.md`、`docs/runbooks/peasd_implementation_guide.md`；
- 实验进度：`experiments/EXPERIMENT_LOG.md`；
- 代码入口见 `02` 文档第 27 节。
