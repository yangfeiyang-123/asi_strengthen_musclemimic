...
**版本日期**：2026-08-11；2026-09-18 收敛修订（只保留正手高远球主线，泛化线归档，运行状态改由 `experiments/EXPERIMENT_LOG.md` 记录）

**执行规范**：[`docs/runbooks/peasd_implementation_guide.md`](../runbooks/peasd_implementation_guide.md)
...
主线只有一个动作：**正手高远球**（registry slug `forehand_clear`），终点为 Stage1 → 完整 Stage2 → H1/H2/H3 Stage3 → formal release。
...
|---|---:|---|---|
| Clear | 22/5 源划分 → aug100 **80/20**（正式唯一数据组，held-out 为 yaw 分组轨迹，见 `experiments/EXPERIMENT_LOG.md`） | 已落盘并通过数值 QC | software movement cue 到 recording stop 的 exploratory 归一化，不声称独立 impact 对齐 |
...
超过 MVC 不构成阻塞。严格遵循 `docs/contracts/MVC小于动作信号时如何处理.md`：
...
5. basis 只对 Clear 拟合并冻结；其他动作不共享。
...
运行状态以 `experiments/EXPERIMENT_LOG.md` 为唯一记录，本文不再复制快照。截至 2026-09-18：

- Stage1 80/20 family 15 个 run 中 12 个到达 800M endpoint（seed0 T0–T4、seed1 T1–T4、seed2 T2–T4），checkpoint 已迁到本机 `checkpoints/stage1/`，尚未晋级；
  缺 T0 s1（本机 2026-09-11 中断于约 300M）、T0 s2、T1 s2；
- Stage2、Stage3 未开始；
- 早期 40/10 子集的 T2/T3/T4 单 seed 320M/640M 结果只作探索性诊断，不进入 formal family。
...
- **数据/任务**：Clear aug100 80/20（22/5 源划分）；15 comparable EMG channels。
...
|---|---|---|---|---:|---|
| M0 | 数据、review、tube、dry-run | Clear data QC/release、verified tube gate、focused tests | 所有 hard errors 为 0；bundle 可重建 | CPU | 核心完成 |
...
- 一张物理 GPU 一个正式训练进程；具体占用以 `experiments/EXPERIMENT_LOG.md` 与 `nvidia-smi` 为准，本文不记录快照。
- 只有空闲、显存占用低且无其他用户进程的 GPU 才能加入正式队列。
- 不在同一 GPU 并发多个 Stage1 任务；这通常不会缩短总 makespan，并会破坏正式资源合同。
...
- ChinaJump / Lift 泛化线（文档已归档至 `docs/archive/`）；
- 双人对打环境（保留为 Stage3 之后的评估场）。
...
5. 每次状态变化更新 [`experiments/EXPERIMENT_LOG.md`](../../experiments/EXPERIMENT_LOG.md)，但不要手写实验指标。
