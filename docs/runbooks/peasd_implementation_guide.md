...
> 本文是 `docs/narrative/` 三份文档在当前仓库中的可执行落地版。主线固定为：
>
...
> 2026-09-18 起主线只保留正手高远球。下文出现的 Lift / ChinaJump 列仅说明接口按 registry 通用，不再排期；
> 运行进度以 `experiments/EXPERIMENT_LOG.md` 为准。
...
v2 tube 严格执行 `docs/contracts/MVC小于动作信号时如何处理.md` 的双轨合同：
...
不熟悉 JSON schema 时，先按 `docs/contracts/emg_human_review_wizard.md` 运行
