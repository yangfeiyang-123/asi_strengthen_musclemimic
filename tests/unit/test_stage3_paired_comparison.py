...
def _selection(tmp_path: Path) -> tuple[Path, dict[str, str]]:
    ...
    return (
        tmp_path / "latent" / "selected" / "selection_manifest.json",
        fingerprints,
    )


def _episode(index: int, offset: float) -> dict:
    return {
        "episode": index,
        "return": 10.0 + offset,
        "hit": True,
...
def _report(
    ...
    *,
    ...
    report = {
        ...
        "evaluation_seed": 123,
        "episodes": [_episode(0, offset), _episode(1, offset)],
...
def test_paired_comparison_seals_shared_feed_seed_and_selection(tmp_path):
    ...
    direct = _report(
        tmp_path,
        family="best_direct",
        latent_fingerprint=fingerprints["best_direct"],
        offset=0.0,
        shared=shared,
    )
    synergy = _report(
